...
function App() {
  ...
  useEffect(() => {
    ...
    const contractNoParam = params.get('contractNo') || params.get('contract') || '';
  ...
  const handleSubmit = async (e) => {
    ...
    if (!contractNo.trim()) {
      alert('계약 번호가 없습니다. URL에 계약 번호를 포함해주세요. (예: ?contractNo=12345)');
      return;
    }
    ...
    try {
      ...
      formData.append('entry.계약번호_ENTRY_ID', contractNo);
      // 배차 요청 내용 필드 (구글 폼의 entry ID로 변경 필요)
      formData.append('entry.배차요청내용_ENTRY_ID', content);
      // 제출 시간 필드 (구글 폼의 entry ID로 변경 필요)
      formData.append('entry.제출시간_ENTRY_ID', submitTime);
        ...
        {!isSubmitted && (
          <div id="desc_area">
            <div className="info_box">
              <h4>📋 사용 안내</h4>
              <p>
                1. URL에 계약 번호를 포함하여 접근하세요. (예: ?contractNo=12345)
                <br />
